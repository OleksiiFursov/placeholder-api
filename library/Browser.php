...
class Browser extends Model
{
    ...
    const BROWSER_INSOMNIA = 'Insomnia'; // https://www.ucweb.com/
     ...
     * @return boolean True if the browser is UCBrowser otherwise false
     */
    protected function checkBrowserInsomnia()
    {
        if (preg_match('/insomnia\/?([\d\.]+)/', $this->_agent, $matches)) {
            if (isset($matches[1])) {
                $this->setVersion($matches[1]);
            }
            if (stripos($this->_agent, 'Mobile') !== false) {
            ...
            $this->setBrowser(self::BROWSER_INSOMNIA);
            return true;
        }
        return false;
    }
